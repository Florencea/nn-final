...
with open('data_name_steps.train', 'w', encoding = 'utf-8-sig') as text_file:
    for rc in receipe:
        if rc['name'] == '' or rc['name'] is None or rc['steps'] == None:
            continue
        label_str = ''
        for ct in rc['category']:
            label_str += '__label__'
...
with open('data_intro_steps.train', 'w', encoding = 'utf-8-sig') as text_file:
    for rc in receipe:
        if rc['intro'] == '' or rc['intro'] is None or rc['steps'] == None:
            continue
        label_str = ''
        for ct in rc['category']:
            label_str += '__label__'
...
with open('data_name_intro_steps.train', 'w', encoding = 'utf-8-sig') as text_file:
    for rc in receipe:
        if rc['name'] == '' or rc['name'] is None or rc['intro'] == '' or rc['intro'] is None or rc['steps'] == None:
            continue
        label_str = ''
        for ct in rc['category']:
            label_str += '__label__'
