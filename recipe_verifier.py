...
if verify_mode:
    ...
    for training_source in training_list:
        with open('result/' + training_source.split('.')[0].replace('data_', 'result_') + '.csv', 'w', encoding='utf-8-sig') as result_csv:
            t = tuple('訓練資料來源 訓練資料筆數 驗證資料筆數 訓練次數 驗證時k值 lr epoch word_ngrams 精確度平均 精確度標準差 召回率平均 召回率標準差'.split(' '))
            print('%s %14s %7s %s %5s %3s %6s %4s %4s %4s %4s %4s' % t, file=result_csv)
            for lr_i in lr_list:
                for ep_i in epoch_list:
                    for wng_i in wng_list:
                        result_p = []
                        ...
                        print('%-16s' % tmp + '%12d' % result_cnt + '%12s' % result_nexample + '%9s' % test_times + '%9s' %  1 + '%8s' %  lr_i + '%5s' % ep_i + '%10s' % wng_i, end='', file=result_csv)
